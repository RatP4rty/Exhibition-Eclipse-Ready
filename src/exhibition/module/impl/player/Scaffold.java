...
extends Module {
    ...
    public void onEnable() {
        Module[] modules;
        for (Module module : modules = new Module[]{(Module)Client.getModuleManager().get(Fly.class), (Module)Client.getModuleManager().get(Speed.class),(Module)Client.getModuleManager().get(LongJump.class)}) {
            ...
            if (!currentMode.equalsIgnoreCase("Watchdog")) {
                ...
                if (em.isPre()) {
                    ...
                    if (this.blockBelowData != null && ((Boolean)((Setting)this.settings.get(this.TOWER)).getValue()).booleanValue() && this.getBlockCount() > 0 && Scaffold.mc.gameSettings.keyBindJump.getIsKeyPressed()) {
                         ...
                         Scaffold.mc.thePlayer.motionY = 0.419;
                ...
                if (em.isPre()) {
                    ...
                    if (Scaffold.mc.gameSettings.keyBindJump.getIsKeyPressed() && this.getBlockCount() > 0 && PlayerUtil.MovementInput()) {
                        if (((Boolean)((Setting)this.settings.get(this.TOWER)).getValue()).booleanValue() && !PlayerUtil.MovementInput()) {
                    ...
                    if (Scaffold.mc.gameSettings.keyBindJump.getIsKeyPressed() && this.getBlockCount() > 0 ) {
                        if (!PlayerUtil.isMoving() || ((Boolean)((Setting)this.settings.get(this.TOWER)).getValue()).booleanValue() && !PlayerUtil.MovementInput()) {
                            ...
                            if (Scaffold.mc.thePlayer.posY != (double)(pos.getY() + 1) && this.material(new BlockPos(Scaffold.mc.thePlayer.posX, Scaffold.mc.thePlayer.posY - 1.2, Scaffold.mc.thePlayer.posZ)) != Material.air ) {
                                if (((Boolean)((Setting)this.settings.get(this.TOWER)).getValue()).booleanValue())  {
                                    ...
                                    Scaffold.mc.thePlayer.motionY = 0.418;
                                }
                                if ( !PlayerUtil.MovementInput() && this.towerTimer.delay(1500.0f)) {
                                    this.towerTimer.reset();
                     ...
                     this.towerTimer.reset();
                            ...
                            if (thing == 0.001 && this.towerTimer.delay(0.0f) && !Scaffold.mc.thePlayer.isPotionActive(Potion.moveSpeed) && PlayerUtil.MovementInput()) {
    ...
    private Vec3 checkPos(BlockPos pos) {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            if (!(eye.squareDistanceTo(vec3.addVector(0.5, 0.75, 0.5)) < eye.squareDistanceTo(vec32.addVector(0.5, 0.5, 0.5))) || !this.blockPos(blockPos).canCollideCheck(this.blockState(blockPos), false)) continue;
    ...
    private boolean place(BlockPos pos) {
        ...
        for (EnumFacing side : EnumFacing.values()) {
            ...
            Vec3 vec3 = new Vec3(pos.getX(), pos.getY(), pos.getZ());
            Vec3 vec32 = new Vec3(blockPos.getX(), blockPos.getY(), blockPos.getZ());
            if (!(eye.squareDistanceTo(vec3.addVector(0.5, 0.75, 0.5)) < eye.squareDistanceTo(vec32.addVector(0.5, -0.5, 0.5))) || !this.blockPos(blockPos).canCollideCheck(this.blockState(blockPos), false) || !(eye.squareDistanceTo(vec = new Vec3(blockPos.getX(), blockPos.getY(), blockPos.getZ()).addVector(0.5, 0.5, 0.5).add(new Vec3(otherSide.getDirectionVec().getX(), otherSide.getDirectionVec().getY(), otherSide.getDirectionVec().getZ()).multi(0.5))) <= 20.0)) continue;
