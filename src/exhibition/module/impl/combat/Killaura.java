   ...
   private float[] facing4;
    ...
    public Killaura(ModuleData data) {
	   ...
	   public void onEvent(Event event) {
	      if (event instanceof EventPacket) {
	         ...
	         if (this.nextRandom == 0) {
	            this.nextRandom = 15 / this.randomNumber(min, max);
	         }
	               ...
	               if (em.isPre()) {
	                  ...
	                  if (loaded.size() > 0) {
	                     ...
	                     if (target != null) {
	                        ...
	                        facing1 = rotations;
	                        facing2 =RotationUtils.getNeededRotations(target);
	                        facing3 = RotationUtils.getRotations5(target);
	                        facing4 = rotations;
	                       switch (randomNumber(1,3)) {
	                        ...
	                        if(facing.length >0) {
	                            em.setYaw((facing[0])+ (float)rand.nextInt(8) -2.0F);
	                        ...
	                        if (block && mc.thePlayer.inventory.getCurrentItem() != null && mc.thePlayer.inventory.getCurrentItem().getItem() instanceof ItemSword && (target != null || !Killaura.loaded.isEmpty() )) {
	                  ...
	                  boolean isAttacking = mc.thePlayer.getDistanceToEntity(target) <= (mc.thePlayer.canEntityBeSeen(target) ? range : 3.0F) && this.delay.delay((float)(50 * this.nextRandom)) && (!mc.thePlayer.onGround || this.isCritSetup || target.hurtResistantTime >= 15);
	                  ...
	                  boolean shouldNotBlock = tooFast && PlayerUtil.isMoving() && mc.thePlayer.isPotionActive(Potion.moveSpeed.id);
	                  
	                  if (mc.thePlayer.isBlocking() && !this.isBlocking && !shouldNotBlock) {
	                      this.isBlocking = true;
	                      this.mc.getNetHandler().addToSendQueue(new C08PacketPlayerBlockPlacement(new BlockPos(-1, -1, -1), 255, Minecraft.getMinecraft().thePlayer.inventory.getCurrentItem(), 0.0f, 0.0f, 0.0f));
	                              }
	               }
                      if (em.isPost() && loaded.isEmpty() && this.isBlocking && this.blockTimer.delay(100.0F) && block) {
	                  this.blockTimer.reset();
	                  this.isBlocking = false;
	   ...
	   public void attack(Entity ent, boolean crits) {
		   ...
		   boolean canCrit = mc.thePlayer.isCollidedVertically && mc.thePlayer.onGround ;
		   if (!((Boolean)this.noswing.getValue()).booleanValue()) {
	         mc.thePlayer.swingItem();
	      } else {
	         mc.thePlayer.swingItemFake();
	   ...
	   private boolean validEntity(EntityLivingBase entity) {
	      ...
	      if (mc.thePlayer.getHealth() > 0.0F && entity.getHealth() > 0.0F && !entity.isDead) {
	         ...
	         if (mc.thePlayer.getDistanceToEntity(entity) <= range && raytrace && entity.ticksExisted > TICK) {
